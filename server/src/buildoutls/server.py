...
def getOptionValue(option: Union[buildout.BuildoutOptionDefinition, str]) -> str:
...
async def lsp_symbols(
  ...
  for section_name, section_value in parsed.items():
    ...
    for option_name, option_value in section_value.items():
      ...
      option_range = Range(
        start=Position(
          line=min(r.range.start.line for r in option_value.locations), character=0
        ),
        end=Position(
          line=max(r.range.end.line for r in option_value.locations), character=0
        ),
      )
    ...
    section_range = Range(
      start=section_header_location.range.start,
        ...
        if children
        else section_header_location.range.end.line,
        character=0,
      ),
    )
          ...
          for k, v in recipe.options.items():
            items.append(
              CompletionItem(
                ...
                kind=CompletionItemKind.Variable,
                tags=([CompletionItemTag.Deprecated] if v.deprecated else []),
                documentation=MarkupContent(
                  kind=MarkupKind.Markdown,
                  value=(
                    f"**Deprecated**\n{v.deprecated}\n\n----\n{v.documentation}"
                    if v.deprecated
                    else v.documentation
                  ),
                ),
              )
            )
      ...
      if symbol.current_option_name == "recipe":
        ...
        for recipe_name, recipe in recipes.registry.items():
          items.append(
            CompletionItem(
              label=recipe_name,
              text_edit=getDefaultTextEdit(doc, params.position, recipe_name),
  ...
  if symbol:
    if symbol.kind in (
      ...
      if symbol.current_option_name == "<":
        ...
        symbol.current_section_name == "buildout"
        and symbol.current_option_name == "extends"
      ):
        extend = symbol.value
...
@server.feature(TEXT_DOCUMENT_REFERENCES, ReferenceOptions(work_done_progress=True))
async def lsp_references(
  ...
  if searched_symbol is not None:
    ...
    logger.debug(
      "Looking for references for %s ${%s:%s}",
      searched_symbol,
      searched_section,
      searched_option,
    )
    assert searched_section
    for profile_path in pathlib.Path(server.workspace.root_path).glob("**/*.cfg"):
      ...
      if profile is not None:
        ...
        if parts is not None and searched_section in parts.value:
          for option_text, option_range in profile.getOptionValues("buildout", "parts"):
        ...
        async for symbol in profile.getAllOptionReferenceSymbols():
          if symbol.referenced_section_name == searched_section:
            if searched_option is None:
              references.append(Location(uri=profile.uri, range=symbol.section_range))
            elif symbol.referenced_option_name == searched_option:
              references.append(Location(uri=profile.uri, range=symbol.option_range))
...
async def lsp_document_link(
  ...
  if "extends" in parsed_buildout["buildout"]:
    for extend, extend_range in parsed_buildout.getOptionValues("buildout", "extends"):
      target = extend
      if target:
        if not buildout._isurl(extend):
